...
class Dataset(data.Dataset):
    def __init__(self, type='train', base_len=50):   # base_len句子长度
        super().__init__()
    ...
    # 句子切分点计算：先以50个字为一个句子，如果切点上正好不是“O”，那就往后移
    def get_points(self):
        self.points = [0]
        i = 0
        while True:
    ...
    # 数据集有多少个句子，也即原train_sample.txt能被切成多少条训练样本
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        input = [self.word2id.get(w, word_unk_id) for w in df['word']]   # 将文字转化为ID
        target = [self.label2id.get(l, label_o_id) for l in df['label']]   # 将标签文字转化为类别ID
        return input, target

# 原本载入的句子可能长度不一致，需要这个函数将句子填充，最终DataLoader出去的每个batch内句子都有一样长度
